...
CSR은 "이 공개키에 대한 인증서를 발급해달라"는 요청서다.
...
브라우저의 동일 출처 정책(Same-Origin Policy)에 의해, `https://frontend.example.com`에서 `https://api.example.com`으로의 AJAX 요청은 기본적으로 차단된다. CORS는 서버가 "이 출처에서의 요청을 허용한다"라고 명시적으로 선언하는 메커니즘이다.
