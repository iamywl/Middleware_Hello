...
이 문서는 **한 권의 책**처럼 기승전결 구조로 설계되었습니다. 처음부터 순서대로 읽으면 Scouter를 "설치하는 사람"에서 "이해하고 활용하는 사람"으로 성장하는 과정을 따라갈 수 있습니다.
...
**읽는 방법:**
- **시간이 충분하다면**: Part 1 → 2 → 3 → 4 → 5 순서대로 읽으세요.
- **빨리 써보고 싶다면**: Part 1 → 3 → 5(실습 1~3) → Part 2 → 4 순서로 읽으세요.
- **이미 APM을 알고 있다면**: Part 2(내부 메커니즘)부터 시작하세요.
...
> Object가 보이지 않으면 Tomcat 컨테이너가 아직 기동 중이거나 Agent 설정에 문제가 있는 것입니다. [트러블슈팅](#31-트러블슈팅) 섹션을 참고하세요.

### Part 1 정리 — 여기까지 온 당신의 현재 위치

Part 1을 마치면 다음 상태가 됩니다:
...
이제 터미널에서 `curl -sk https://localhost/health`를 실행하면 Scouter Client의 XLog 화면에 **파란 점 하나**가 찍힙니다. 축하합니다 — Scouter가 동작하고 있습니다.

하지만 이 시점에서 떠오르는 질문이 있을 것입니다:
- "소스코드를 한 줄도 안 고쳤는데, **어떻게** 요청을 감지한 거지?"
- "데이터가 Agent에서 Server로 **어떻게** 전달된 거지?"
- "그 점의 **위치**(X축, Y축)는 어떻게 결정된 거지?"

Part 2에서 이 질문들에 정확히 답합니다.

---

# Part 2. 내부 동작 메커니즘

> Part 1에서 Scouter를 설치하고 Client를 접속했습니다. Object 패널에 tomcat1, tomcat2가 보이고, curl을 치면 XLog에 점이 찍힙니다. **하지만 여기서 멈추면 Scouter는 "그냥 그래프 보는 도구"로 끝납니다.**
>
> Part 2에서는 그 점 하나가 **어떻게** 만들어지는지를 추적합니다. Agent가 바이트코드를 어떻게 변조하고(6장), 수집한 데이터를 어떤 프로토콜로 Server에 보내며(7장), Server가 이를 어떻게 가공·저장하는지(8~11장)를 이해하면, 운영 환경에서 "XLog에 점이 안 찍힌다", "Counter 그래프가 갑자기 끊겼다" 같은 문제를 **원인 수준에서** 진단할 수 있습니다.
>
> 즉, Part 2는 **"Scouter를 쓸 줄 아는 사람"에서 "Scouter를 이해하는 사람"으로 넘어가는 단계**입니다.

## 6. Bytecode Instrumentation — Agent의 핵심 원리

이 장은 Scouter Agent의 핵심 기술인 **Java Bytecode Instrumentation(BCI)**을 JVM 내부 구조부터 Opcode 수준까지 설명합니다. 이 장을 이해하면 "소스코드를 수정하지 않고 어떻게 성능을 측정하는가?"라는 질문에 정확히 답할 수 있습니다.

앞서 Part 1의 docker-compose.yml에서 `-javaagent:scouter.agent.jar` 옵션을 봤습니다. 이 한 줄의 JVM 인자가 Scouter의 모든 동작의 출발점이며, 이 장에서는 그 한 줄이 JVM 내부에서 정확히 무슨 일을 일으키는지를 파헤칩니다.

### 6.1 전제 지식: JVM은 스택 기반 가상 머신이다

JVM은 CPU가 아닙니다. Java 소스코드(.java)는 `javac` 컴파일러에 의해 **바이트코드(.class)**로 변환되고, JVM이 이 바이트코드를 해석(Interpret)하거나 네이티브 코드로 컴파일(JIT)하여 실행합니다.

바이트코드는 **JVM 명령어 세트(Instruction Set)**로 구성됩니다. x86 CPU가 `MOV`, `ADD`, `CALL` 같은 기계어 명령어를 실행하듯, JVM은 `ALOAD`, `INVOKEVIRTUAL`, `ARETURN` 같은 **Opcode**를 실행합니다.

핵심적인 차이: JVM은 **레지스터가 아닌 스택(Operand Stack)**을 사용합니다.
...
**중요**: 메서드의 실행 코드는 `Code Attribute` 안의 `bytecode[]` 배열입니다. Scouter Agent가 변조하는 것이 바로 이 바이트 배열입니다.

### 6.3 실제 바이트코드 예시: health() 메서드

`HealthController.health()` 메서드를 `javap -c`로 역어셈블하면 다음과 같습니다:
...
각 숫자(0, 2, 4, 6, ...)는 `bytecode[]` 배열의 오프셋(인덱스)입니다. JVM의 실행 엔진은 이 배열을 순차적으로 읽으며 명령을 수행합니다.
...
JVM이 코드를 실행하려면 `.class` 파일을 메모리에 로드해야 합니다. 이 과정은 3단계로 구성됩니다:
...
**Scouter Agent가 개입하는 시점은 ①과 ② 사이**입니다. Loading 단계에서 ClassLoader가 `byte[]`를 읽어온 직후, 그 `byte[]`를 JVM에 넘기기 직전에 가로채서 변조합니다.

### 6.5 java.lang.instrument API — Agent의 법적 근거

JVM은 임의의 코드가 클래스 로딩에 개입하는 것을 허용하지 않습니다. **`java.lang.instrument`** 패키지는 JVM이 공식적으로 제공하는 유일한 바이트코드 변조 경로입니다.
...
**핵심**: `inst.addTransformer()`를 호출하는 순간, 이후 JVM에서 로드되는 **모든 클래스**에 대해 등록된 Transformer의 `transform()` 메서드가 콜백됩니다.
...
이 인터페이스의 구현이 Scouter Agent의 핵심입니다:
...
ASM은 `.class` 파일의 바이너리를 파싱하고 편집하는 라이브러리입니다. Scouter뿐 아니라 Spring Framework, Hibernate, Gradle도 내부적으로 ASM을 사용합니다.

#### 6.6.1 ASM의 Visitor 패턴

ASM은 **Visitor 패턴**으로 바이트코드를 처리합니다. `.class` 파일을 한 바이트씩 순차적으로 읽으면서, 구조적 요소(클래스, 필드, 메서드, 명령어)를 만날 때마다 대응하는 Visitor 메서드를 콜백합니다.
...
Scouter의 HTTP 서비스 Hook은 다음과 같이 구현됩니다:
...
위 ASM 코드가 실행되면, `HttpServlet.service()` 메서드의 바이트코드가 다음과 같이 변합니다:
...
이 세 가지가 정확하지 않으면 JVM의 Verification 단계에서 `VerifyError`가 발생하고 클래스 로딩이 실패합니다. ASM의 `ClassWriter.COMPUTE_FRAMES` 옵션이 이를 자동 처리합니다.

### 6.7 TraceContext와 ThreadLocal — 요청 추적의 핵심

바이트코드 삽입은 "언제 시작하고 언제 끝나는지"를 기록할 뿐입니다. 하나의 HTTP 요청이 여러 메서드를 거치는 동안 **동일한 컨텍스트**를 유지하는 것은 `ThreadLocal`이 담당합니다.
...
`ThreadLocal`은 JVM의 스레드별 격리 저장소입니다. 동시에 수십 개의 HTTP 요청이 처리되더라도, 각 스레드의 TraceContext는 서로 간섭하지 않습니다. 이것이 Scouter가 멀티스레드 환경에서 안전하게 동작하는 근거입니다.

### 6.8 Hook 대상 결정 메커니즘

`transform()` 메서드는 JVM이 로드하는 **모든 클래스**에 대해 호출됩니다. 하지만 모든 클래스를 변조하면 오버헤드가 발생하므로, Scouter는 두 가지 기준으로 Hook 대상을 결정합니다.
...
JDBC Hook의 경우, `java.sql.Connection`은 인터페이스이므로 실제로는 MySQL 드라이버의 `com.mysql.cj.jdbc.ConnectionImpl` 같은 구현 클래스가 로드될 때 해당 클래스의 `prepareStatement()` 메서드를 Hook합니다.
...
패턴 매칭은 `transform()` 메서드 내에서 className을 `.`을 `/`로 치환하고, 와일드카드(`*`)를 정규식으로 변환하여 수행합니다.
...
두 설정의 차이: `hook_service_patterns`은 `TraceContextManager.startTrace()`를 호출하고, `hook_method_patterns`은 이미 존재하는 TraceContext에 Step을 추가할 뿐입니다.
...
JIT 컴파일의 결과: 반복 실행되는 메서드에서 Scouter의 측정 코드는 **네이티브 기계어 수준에서 최적화**됩니다. 따라서 바이트코드 인터프리터 모드보다 오버헤드가 더 줄어듭니다.
...
이 오버헤드는 클래스 로딩 시 1회만 발생합니다. 애플리케이션 기동 시간에 미미한 영향을 줍니다.
...
응답시간이 10ms인 요청에 대해 2μs의 오버헤드는 **0.02%**입니다.
...
6장에서 Agent가 바이트코드 변조를 통해 **요청의 시작/종료 시간, URL, SQL 등을 수집**하는 과정을 살펴보았습니다. 그런데 수집한 데이터가 Agent의 JVM 메모리에만 남아 있으면 소용이 없습니다. Scouter Client에서 그래프를 보려면, 이 데이터가 **네트워크를 통해 Server로 전달**되어야 합니다.

이 장에서는 Agent가 수집한 데이터를 Server로 보내는 **통신 프로토콜**을 다룹니다. 왜 UDP와 TCP를 동시에 사용하는지, 데이터를 어떤 형태로 직렬화하는지, 네트워크 대역폭을 어떻게 절약하는지를 이해하면, "Agent는 연결됐는데 XLog만 안 뜬다"거나 "Counter는 보이는데 Profile이 안 열린다" 같은 증상의 원인을 정확히 짚을 수 있습니다.
...
Scouter는 모든 데이터를 **Pack**이라는 직렬화 단위로 포장하여 전송합니다. Pack은 Scouter가 자체 정의한 바이너리 프로토콜로, JSON이나 Protobuf가 아닌 **커스텀 직렬화 포맷**입니다.
...
**DataOutputX / DataInputX**: Scouter의 커스텀 직렬화 유틸리티입니다. Java 표준의 `ObjectOutputStream`을 사용하지 않는 이유는 성능입니다. `ObjectOutputStream`은 클래스 메타데이터, 직렬화 버전 UID 등의 오버헤드가 크지만, DataOutputX는 필드를 원시 바이트로 직접 쓰므로 패킷 크기가 최소화됩니다.
...
`udpSocket.send()`는 커널의 송신 버퍼에 데이터를 복사하고 즉시 반환합니다. TCP처럼 상대방의 ACK를 기다리지 않으므로, 호출 스레드가 블로킹되지 않습니다. 이것이 Agent의 오버헤드를 최소화하는 핵심입니다.
...
성능 카운터는 2초마다 새 값이 오므로, 한 건이 유실되어도 다음 주기의 값으로 대체됩니다. XLog도 마찬가지로, TPS가 극도로 높은 상황에서 일부가 유실되어도 전체 패턴 분석에는 지장이 없습니다.

### 7.4 TCP 통신 — 신뢰성 필요 데이터

TCP로 전송되는 데이터는 **유실되면 복구할 수 없는** 것들입니다:
...
TCP 연결은 Agent 시작 시 1개만 생성하고, 이후 모든 TCP 통신에 재사용합니다. 연결이 끊기면 자동 재연결을 시도합니다.

### 7.5 Text Dictionary — 대역폭 최적화

Scouter는 **Text Dictionary** 기법으로 네트워크 대역폭을 절약합니다. 이는 HTTP/2의 HPACK 헤더 압축과 유사한 원리입니다.
...
본 프로젝트에서는 Agent와 Server가 동일 Docker 네트워크(`mw-network`)에 있으므로 ②의 확률은 사실상 0입니다. ③이 문제가 되는 것은 Agent가 10대 이상이고 TPS가 수만인 대규모 환경입니다.
...
6장에서 Agent가 바이트코드를 변조하여 성능 데이터를 **수집**하는 과정을, 7장에서 그 데이터를 Server로 **전송**하는 프로토콜을 살펴보았습니다. 이 장에서는 이 두 과정을 하나로 엮어, **사용자의 curl 명령 한 번이 XLog 점 하나가 되기까지의 전체 여정**을 시간순으로 따라갑니다.

이 파이프라인을 한 눈에 이해하면, "데이터가 어디서 누락되었는가"를 단계별로 체크할 수 있는 **트러블슈팅 지도**를 갖게 됩니다.
...
8장에서 HTTP 요청이 XLog Pack으로 변환되어 Server에 도달하는 전체 흐름을 보았습니다. 이 장에서는 그 **XLog Pack 자체**에 집중합니다. Pack 안에 어떤 필드가 담겨 있고, 그 필드들이 Client 화면에서 어떤 시각적 요소(점의 위치, 색상, 클릭 시 프로파일)로 매핑되는지를 구체적으로 설명합니다.

XLog의 내부 구조를 알면, Part 3에서 다룰 XLog 차트의 모든 동작을 "왜 그렇게 보이는지" 수준에서 이해할 수 있습니다.
...
XLogPack의 `txid`, `gxid`, `caller` 3개 필드는 **분산 추적(Distributed Tracing)**을 위한 것입니다.
...
`trace_interservice_enabled=true`로 설정하면, Agent는 외부 HTTP 요청을 보낼 때 `X-Scouter-Gxid`와 `X-Scouter-Caller` 헤더를 자동 삽입합니다. 수신 측 Agent는 이 헤더를 읽어 `gxid`와 `caller` 필드를 채웁니다.
...
XLog 차트에서 점의 좌표는 XLogPack의 필드로 직접 결정됩니다:
...
XLog의 점을 클릭하면 해당 txid에 대한 **Profile Steps**를 Server에 TCP로 요청합니다. Profile Step은 하나의 트랜잭션 내부에서 발생한 이벤트의 시계열 기록입니다.
...
Profile Step들은 `ArrayList<Step>`에 순서대로 쌓이며, `profile_step_max_count=1024` 설정에 의해 하나의 트랜잭션 내에 최대 1024개의 Step만 기록됩니다. 이 제한은 무한 루프나 대량 SQL 실행 시 메모리를 보호합니다.
...
9장까지는 **개별 HTTP 요청**(XLog)의 데이터 흐름을 추적했습니다. 하지만 APM에는 "이 요청이 느렸다"는 개별 추적 외에, "지금 시스템 전체가 어떤 상태인가"를 보여주는 **종합 지표**도 필요합니다.

이 장에서 다루는 Counter가 바로 그 역할입니다. TPS, Heap 사용량, GC 횟수, Active Service 등 **시스템 수준의 수치 데이터**가 어떻게 수집되고 전송되는지를 설명합니다. Part 3에서 TPS 차트, Heap 차트, GC 차트를 볼 때, 그 그래프의 데이터가 정확히 여기서 온다는 것을 알게 됩니다.
...
9장(XLog)과 10장(Counter)에서 Agent가 데이터를 수집하고 Server로 전송하는 과정을 보았습니다. Server는 이 데이터를 **수신만 하는 것이 아니라, 저장해야** 합니다. 실시간 모니터링뿐 아니라, "어제 오후 3시의 XLog를 다시 보고 싶다"는 **과거 조회**가 가능해야 하기 때문입니다.
...
지금까지 데이터의 흐름(수집→전송→저장)을 추적했습니다. 하지만 이 모든 데이터에는 **"어디서 왔는가"**라는 출처 정보가 붙어야 합니다. TPS 45라는 숫자가 tomcat1의 것인지 tomcat2의 것인지 구분할 수 없다면 의미가 없습니다.

Scouter에서 이 "모니터링 대상의 식별과 상태 관리"를 담당하는 것이 **Object**입니다. Client 좌측 패널에 보이는 tomcat1, tomcat2 각각이 하나의 Object이며, 이 장에서는 Object가 어떻게 등록되고, 어떻게 죽은 것으로 판정되며, 컨테이너 재빌드 시 왜 중복으로 보이는지를 설명합니다.
...
Part 2의 마지막 장입니다. 6~12장에서 BCI, 통신 프로토콜, 데이터 파이프라인, 저장 구조, Object 관리를 각각 개별적으로 다루었습니다. 이 장에서는 그 모든 것이 **소스코드 수준에서 어떻게 조직되어 있는지**를 한 눈에 정리합니다. 앞에서 설명한 `TraceMain`, `DataProxy`, `XLogHandler` 같은 클래스들이 실제 코드에서 어디에 위치하고 어떤 의존 관계를 맺는지를 보여줌으로써, Part 2 전체를 하나의 그림으로 통합합니다.

Scouter의 동작을 깊이 이해하려면 소스코드의 패키지 구조와 핵심 클래스 간의 관계를 알아야 합니다. Scouter는 GitHub(`scouter-project/scouter`)에서 Apache 2.0 라이선스로 공개되어 있으며, Java로 작성되었습니다.
...
이 구조에서 모든 ASM 클래스는 `TraceMain`의 static 메서드를 호출하는 코드를 삽입하고, `TraceMain`은 `TraceContextManager`를 통해 현재 스레드의 컨텍스트를 관리하며, 트랜잭션 종료 시 `DataProxy`를 통해 Server로 전송합니다.

### Part 2 정리 — "점 하나"의 일생을 완전히 이해했다

Part 2를 마치면, Part 1에서 봤던 XLog의 파란 점 하나가 어떤 여정을 거쳐 화면에 나타났는지를 **엔드-투-엔드**로 설명할 수 있습니다:
...
이제 Part 3에서 이 데이터가 **Client 화면에서 어떤 형태로 시각화되는지**를 하나씩 살펴봅니다.

---

# Part 3. 모니터링 뷰 완전 가이드

> Part 2에서 Scouter의 내부 동작 메커니즘을 이해했습니다. Agent가 바이트코드를 변조하여 데이터를 수집하고, UDP/TCP로 Server에 전송하며, Server가 이를 파일 DB에 저장하는 전체 파이프라인을 알게 되었습니다.
>
...
XLog는 Scouter(그리고 Jennifer)의 **가장 핵심적인 뷰**입니다. 다른 뷰는 못 열어도 XLog만 열어두면 시스템 상태의 80%를 파악할 수 있습니다.
...
Part 2의 9장에서 다뤘듯, 점의 좌표는 XLogPack의 필드로 직접 결정됩니다:
...
XLog의 진짜 힘은 **점을 클릭했을 때** 발휘됩니다. 점을 클릭하면 해당 요청의 **전체 처리 과정**(Profile)이 시간순으로 펼쳐집니다:
...
> **APM의 핵심 가치**: 로그 파일을 grep하는 대신, **점 하나를 클릭**하면 "이 요청이 왜 느렸는지"가 SQL 레벨, 메서드 레벨에서 바로 보입니다. 이것이 인프라 모니터링(Prometheus/Grafana)과 APM의 결정적 차이입니다.

### 14.5 XLog 드래그 선택

차트에서 영역을 **드래그**하면 해당 시간대+응답시간 범위의 요청만 필터링하여 목록으로 볼 수 있습니다.
...
이 기법은 Jennifer에서도 동일하게 사용되며, APM 분석의 가장 기본적이고 강력한 도구입니다.

---

## 15. TPS 모니터링

14장의 XLog가 **개별 요청을 점으로** 보여준다면, TPS는 그 점들을 **초 단위로 세어서 선(line)으로** 보여줍니다. "지금 초당 몇 건의 요청이 처리되고 있는가"라는 시스템의 처리 용량을 한눈에 파악하는 지표입니다.
...
TPS가 "완료된 요청의 수"라면, Active Service는 "아직 완료되지 않은, **지금 처리 중인 요청의 수**"입니다. TPS가 높더라도 Active Service가 낮으면 서버가 빠르게 소화하고 있다는 뜻이고, TPS는 같은데 Active Service가 높아지면 **요청이 밀리고 있다**는 경고 신호입니다. 두 지표를 함께 봐야 시스템 상태를 정확히 판단할 수 있습니다.
...
**Case 3이 가장 위험**합니다. 요청은 들어오는데 처리가 안 되고 있다는 뜻이므로, 즉시 Thread Dump를 떠서 쓰레드가 어디에 걸려 있는지 확인해야 합니다.
...
TPS와 Active Service가 **요청 수준의 지표**라면, 힙 메모리는 **JVM 자원 수준의 지표**입니다. 요청을 처리할 때마다 Java 객체가 힙에 생성되고, 힙이 가득 차면 GC가 발생하여 응답 지연을 일으킵니다. 따라서 힙 메모리 모니터링은 "왜 갑자기 느려졌는가?"의 원인을 추적하는 출발점이 됩니다.
...
**실전 팁**: Heap Used 차트를 볼 때는 반드시 **GC Time 차트를 옆에 나란히 열어두세요**. Heap이 급락하는 시점이 GC가 실행된 시점이고, 그 GC에 얼마나 걸렸는지가 응답 지연의 직접 원인이 됩니다. 다음 장에서 이 관계를 자세히 다룹니다.

---

## 18. GC 모니터링

17장에서 힙 메모리의 "톱니 모양 패턴"을 언급했습니다. 그 톱니의 급락 지점이 바로 **GC(Garbage Collection)**가 실행된 시점입니다. GC 모니터링은 힙 메모리와 짝으로 봐야 완전한 그림이 됩니다 — 힙이 "얼마나 쌓였는가"를 보여준다면, GC는 "정리에 얼마나 걸렸는가"를 보여줍니다.
...
> **한 줄 요약**: "XLog에서 주기적으로 점이 올라간다" → Heap/GC를 먼저 확인하세요. GC가 원인이면 JVM 튜닝으로 해결되고, GC가 아니면 코드 레벨(SQL, 외부 API)을 봐야 합니다.

---

## 19. 쓰레드 분석

지금까지 XLog, TPS, Active Service, Heap, GC 등의 **수치 그래프**로 시스템 상태를 관찰했습니다. 하지만 이 그래프들이 "무언가 느리다"는 현상을 보여줄 뿐, **왜 느린지**는 알려주지 않습니다. 쓰레드 분석은 "지금 이 순간 서버가 정확히 무엇을 하고 있는가"를 코드 레벨(스택트레이스)에서 보여주는 진단 도구입니다. Active Service가 높은데 원인을 모를 때, Thread Dump를 뜨면 답이 보입니다.
...
Thread Dump를 처음 보면 수백 줄의 스택트레이스에 압도됩니다. 다음 순서로 읽으면 효율적입니다:
...
19장의 Thread Dump가 "이 스레드가 DB 관련 코드에 머물러 있다"고 알려줬다면, 다음 질문은 "**어떤 SQL이 느린가?**"입니다. SQL 추적은 6장에서 다룬 JDBC Hook이 실제로 어떤 데이터를 보여주는지를 다룹니다. XLog 점을 클릭하면 Profile에서 실행된 SQL문과 소요 시간을 바로 확인할 수 있습니다.
...
**실전에서 가장 많이 쓰는 진단 패턴**은 다음과 같습니다:
...
> 본 프로젝트의 `/health` 엔드포인트는 DB를 사용하지 않으므로 SQL Step이 나타나지 않습니다. DB가 포함된 프로젝트에서는 이 진단 흐름이 장애 분석의 핵심 도구가 됩니다.

---

## 21. Active Service EQ (이퀄라이저)

16장에서 Active Service를 **숫자(선 그래프)**로 보았습니다. Active Service EQ는 같은 데이터를 **다른 방식으로 시각화**합니다 — 현재 처리 중인 요청들의 경과 시간을 색상 막대로 표현하여, "몇 개가 처리 중인가"뿐 아니라 "**얼마나 오래 걸리고 있는가**"까지 한눈에 보여줍니다. Jennifer APM에서도 동일한 EQ 뷰가 핵심 모니터링 도구로 사용됩니다.
...
Part 3에서 9개의 모니터링 뷰를 살펴보았습니다. 각 뷰를 개별적으로 아는 것도 중요하지만, 진짜 모니터링 역량은 **뷰 간의 관계**에서 나옵니다. 장애 상황에서의 진단 흐름을 정리하면:
...
이 흐름이 **몸에 익을 때까지** Part 5의 실습을 반복하는 것을 권장합니다.

---

# Part 4. 실전 활용

> Part 3에서 개별 뷰의 의미와 읽는 법을 익혔습니다. 하지만 실제 운영 환경에서는 뷰 하나만 보고 판단하지 않습니다. **여러 뷰를 동시에 열어 놓고, 지표 간의 상관관계를 읽어내는 것**이 APM 모니터링의 핵심 역량입니다.
>
> Part 4에서는 대시보드 구성(23장), 부하 테스트와 모니터링의 결합(24장), 알림 설정(25장)을 통해 Scouter를 **실전에서 활용하는 방법**을 다룹니다. 또한 Prometheus/Grafana, Jennifer와의 비교(28~29장)를 통해 Scouter의 위치를 객관적으로 파악하고, 프로덕션 환경 튜닝(30장)과 트러블슈팅(31장)으로 운영 레벨의 노하우를 정리합니다.
...
23장에서 대시보드를 구성했다면, 이제 **대시보드에 생명을 불어넣을 차례**입니다. 아무 요청도 없으면 그래프는 평평한 선일 뿐이니까요. 부하를 주면서 그래프가 어떻게 반응하는지를 관찰하는 것이 Scouter 학습의 가장 효과적인 방법입니다.
...
24장에서 부하를 주면서 그래프 변화를 직접 관찰했습니다. 하지만 운영 환경에서 24시간 화면을 주시할 수는 없습니다. **알림(Alert)**은 "문제가 생기면 알려주세요"라고 Scouter에게 맡기는 기능입니다. 응답시간이 기준을 넘거나, GC가 오래 걸리거나, CPU가 치솟으면 자동으로 알림이 발생합니다.
...
Part 2의 6장에서 Agent가 `agent.conf`의 `hook_service_patterns`, `hook_method_patterns` 등을 읽어 Hook 대상을 결정한다고 설명했습니다. 이 장에서는 그 설정 파일의 **전체 항목**을 레퍼런스로 정리합니다. 필요할 때 찾아보는 사전처럼 활용하세요.
...
여기까지 오면 자연스럽게 드는 질문이 있습니다: "우리 프로젝트에는 이미 Prometheus/Grafana가 있는데, Scouter는 왜 필요한가?" 이 장에서는 두 도구의 **아키텍처, 기능, 적합한 상황**을 비교하여 답합니다. 결론부터 말하면, 둘은 경쟁이 아니라 **보완 관계**입니다.
...
28장에서 Scouter와 Prometheus/Grafana의 차이를 보았습니다. 이번에는 Scouter의 **직계 조상**인 Jennifer와 비교합니다. Scouter는 Jennifer 핵심 개발자가 오픈소스로 만든 도구이므로, 개념과 용어가 거의 동일합니다. **Scouter를 능숙하게 다루면, Jennifer를 쓰는 현업에 바로 투입될 수 있습니다.**
...
28~29장에서 Scouter가 Prometheus/Grafana, Jennifer와 어떻게 다른지를 비교했습니다. 이제 Scouter를 실제로 **운영 환경에 투입**할 때 고려해야 할 사항을 다룹니다. 본 프로젝트의 Docker Compose 환경은 Agent 2대, TPS 수십 수준이지만, 프로덕션에서는 Agent 수십 대, TPS 수천 이상인 환경을 대비해야 합니다. 이 장에서는 샘플링 전략, Server 스케일링, 네트워크 분리, 보안 고려사항을 정리합니다.

### 30.1 대용량 트래픽 환경의 샘플링 전략

TPS가 1,000을 넘는 환경에서는 모든 요청을 수집하면 Server의 디스크 I/O와 네트워크 대역폭에 부담이 됩니다. Scouter는 **다단계 샘플링**을 제공합니다.
...
**에러 요청은 샘플링에서 제외**: `xlog_sampling_exclude_patterns`에 에러 패턴을 등록하면, 에러 응답은 TPS와 무관하게 항상 수집됩니다. 장애 분석에 필수적인 데이터를 놓치지 않기 위함입니다.
...
UDP는 방화벽/NAT 환경에서 문제가 될 수 있습니다. 이 경우 `net_udp_packet_max_bytes`를 MTU(1500) 이하로 설정하여 IP fragmentation을 방지합니다:
...
운영 환경에서 단일 트랜잭션이 수천 개의 SQL을 실행하는 경우(배치 작업 등), Profile Step 메모리가 급증할 수 있습니다.
...
**Container ID 변경 문제**: 컨테이너가 재생성되면 `obj_name`이 같더라도 내부적으로 다른 Object로 인식될 수 있습니다. `-Dobj_name=tomcat1`처럼 고정된 이름을 JVM 인자로 지정하면 해결됩니다.
...
이렇게 하면 Pod 이름(`app-deployment-7b8c9d-x4k2j`)이 Object 이름이 되어, 스케일 아웃/인 시 각 Pod을 구분할 수 있습니다.

---

## 31. 트러블슈팅

Scouter를 처음 세팅하거나 운영 중에 만나는 **흔한 문제들과 해결 방법**을 정리합니다. 대부분의 문제는 Part 2에서 다룬 통신 구조(UDP/TCP 포트), Object 생명주기(Heartbeat 30초), 바이트코드 Hook 대상(agent.conf 패턴)에 대한 이해만 있으면 원인을 빠르게 특정할 수 있습니다.
...
이제 남은 것은 **직접 해보는 것**입니다. Part 5의 실습 과제로 넘어갑니다.

---

# Part 5. 실습 과제

> Part 1~4에서 Scouter의 설치, 내부 동작, 뷰 해석, 실전 활용을 모두 다루었습니다. 하지만 모니터링은 **읽기만으로는 체득할 수 없는 기술**입니다. 그래프 패턴을 눈으로 보고, 변화의 원인을 추론하고, 스스로 검증하는 경험이 있어야 실제 장애 상황에서 빠르게 대응할 수 있습니다.
>
> Part 5의 12개 실습은 난이도 순으로 구성되어 있으며, 앞선 Part들에서 배운 지식을 **직접 손으로 확인**합니다. 실습 1에서 첫 XLog 점을 찍어보는 것부터, 실습 12에서 장애 시뮬레이션과 진단 보고서 작성까지, 점진적으로 역량을 쌓아갑니다.
>
...
이 문서를 끝까지 읽고 실습을 완료했다면, 당신은 다음을 할 수 있는 사람이 되었습니다:

1. **Scouter를 설치하고 운영할 수 있다** — Agent-Server-Client 구조를 이해하고 직접 배포할 수 있습니다.
2. **내부 동작을 설명할 수 있다** — "소스코드 수정 없이 어떻게 성능을 측정하나요?"라는 질문에 BCI, ASM, ThreadLocal까지 답할 수 있습니다.
3. **그래프를 읽고 진단할 수 있다** — XLog 패턴, TPS/Active Service 관계, GC-응답시간 상관관계를 보고 원인을 추론할 수 있습니다.
4. **장애를 체계적으로 대응할 수 있다** — 감지 → 진단 → 복구 → 확인의 사이클을 수행할 수 있습니다.
5. **Jennifer 기반 환경에 바로 적응할 수 있다** — 용어, 개념, 진단 흐름이 동일하므로 현업 투입이 가능합니다.

APM 모니터링은 **이론만으로는 완성되지 않는 기술**입니다. 부하를 주고, 그래프가 어떻게 반응하는지 관찰하고, 장애를 만들어 보고, 진단해 보는 경험을 반복할수록 실력이 쌓입니다. 실습 과제를 가능한 한 모두 수행해 보시기 바랍니다.
