...
# ─────────────────────────────────────────
# 시나리오 6: 대시보드 종합 (모든 그래프 활성화)
# ─────────────────────────────────────────
scenario_dashboard() {
    print_header "시나리오 6: 대시보드 종합 — 모든 그래프를 움직여라"
    echo "  목적: Scouter + Grafana + Prometheus의 모든 패널이 반응하도록"
    echo "        다양한 부하 패턴을 2분간 순차적으로 주입"
    echo ""
    echo "  ┌──────────────────────────────────────────────────────────┐"
    echo "  │  지금 열어야 할 화면:                                      │"
    echo "  │  1. Scouter Client (XLog, TPS, Heap, Elapsed, Active EQ) │"
    echo "  │  2. Grafana http://localhost:3000 (Middleware Overview)   │"
    echo "  │  3. (선택) Prometheus http://localhost:9090               │"
    echo "  └──────────────────────────────────────────────────────────┘"
    echo ""
    echo -e "${YELLOW}  10초 후 시작합니다... 모니터링 화면을 준비하세요${NC}"
    sleep 10

    # ── Phase 1: 워밍업 (Scouter: XLog 점 생성, Grafana: 요청 카운트 시작) ──
    print_header "Phase 1/7: 워밍업 — 기본 트래픽 (20초)"
    echo "  → Scouter XLog에 점이 나타나기 시작합니다"
    echo "  → Grafana Nginx Requests/sec가 올라갑니다"
    echo ""
    local end_time=$((SECONDS + 20))
    while [ $SECONDS -lt $end_time ]; do
        seq 1 10 | xargs -P 5 -I {} \
            curl -sk -o /dev/null "${BASE_URL}/health" 2>/dev/null
        sleep 0.5
    done
    print_result "Phase 1 완료 — 베이스라인 트래픽 생성됨"

    # ── Phase 2: 혼합 엔드포인트 (다양한 응답 크기 → Network I/O) ──
    echo ""
    print_header "Phase 2/7: 혼합 엔드포인트 — 응답 크기 다양화 (20초)"
    echo "  → Grafana Network I/O 그래프가 움직입니다"
    echo "  → Scouter XLog에 다양한 응답시간의 점이 나타납니다"
    echo ""
    local endpoints=(
        "${BASE_URL}/"
        "${BASE_URL}/health"
    ...
    while [ $SECONDS -lt $end_time ]; do
        for url in "${endpoints[@]}"; do
            curl -sk -o /dev/null "$url" 2>/dev/null &
        done
        wait
        sleep 0.3
    done
    print_result "Phase 2 완료 — 6개 엔드포인트 혼합 트래픽"

    # ── Phase 3: 동시성 계단 증가 (Active Connections, Active Service EQ) ──
    echo ""
    print_header "Phase 3/7: 동시성 계단 — C=5→20→50→100 (40초)"
    echo "  → Grafana Nginx Active Connections 계단식 증가"
    echo "  → Scouter Active Service EQ 막대가 커집니다"
    echo "  → Scouter Elapsed Time이 점점 올라갑니다"
    echo ""
    local levels=(5 20 50 100)
    for c in "${levels[@]}"; do
        echo -e "  ${CYAN}→ 동시성 C=${c} (10초간)${NC}"
        end_time=$((SECONDS + 10))
        while [ $SECONDS -lt $end_time ]; do
            seq 1 "$c" | xargs -P "$c" -I {} \
                curl -sk -o /dev/null "${BASE_URL}/info" 2>/dev/null
        done
    done
    print_result "Phase 3 완료 — 동시성 계단 테스트"

    # ── Phase 4: 순간 폭주 (CPU 스파이크, TPS 급등) ──
    echo ""
    print_header "Phase 4/7: 순간 폭주 — 500회 동시 100 (CPU 스파이크)"
    echo "  → Grafana CPU Usage가 치솟습니다"
    echo "  → Scouter TPS가 급등합니다"
    echo "  → Scouter Heap Used가 급격히 올라갑니다"
    echo ""
    seq 1 500 | xargs -P 100 -I {} \
        curl -sk -o /dev/null "${BASE_URL}/info" 2>/dev/null
    print_result "Phase 4 완료 — CPU/TPS 스파이크 생성됨"

    # ── Phase 5: 대용량 응답 (Network TX, Prometheus 메트릭) ──
    echo ""
    print_header "Phase 5/7: 대용량 응답 — /actuator/prometheus 집중 (15초)"
    echo "  → /actuator/prometheus는 수천 줄의 메트릭 텍스트를 반환"
    echo "  → Grafana Network I/O TX(송신)가 확 올라갑니다"
    echo ""
    end_time=$((SECONDS + 15))
    while [ $SECONDS -lt $end_time ]; do
        seq 1 20 | xargs -P 20 -I {} \
            curl -sk -o /dev/null "${BASE_URL}/actuator/prometheus" 2>/dev/null
        sleep 0.2
    done
    print_result "Phase 5 완료 — 네트워크 I/O 부하 생성됨"

    # ── Phase 6: 휴식 → GC 관찰 (Heap Used 톱니 패턴) ──
    echo ""
    print_header "Phase 6/7: 휴식 — GC 관찰 (20초)"
    echo "  → 트래픽을 멈추고 JVM이 메모리를 회수하는지 관찰"
    echo "  → Scouter Heap Used가 톱니처럼 뚝 떨어지면 GC 정상"
    echo "  → Grafana JVM Heap 그래프도 하락"
    echo "  → Scouter TPS가 0으로 떨어짐"
    echo ""
    local wait_secs=20
    for i in $(seq $wait_secs -5 5); do
        echo "    ${i}초 남음..."
        sleep 5
    done
    print_result "Phase 6 완료 — GC 회수 패턴 관찰 구간"

    # ── Phase 7: 재부하 (회복 확인) ──
    echo ""
    print_header "Phase 7/7: 재부하 — 회복 확인 (15초)"
    echo "  → 휴식 후 다시 부하를 주어 시스템이 정상 복귀했는지 확인"
    echo "  → 모든 그래프가 다시 활성화됩니다"
    echo ""
    end_time=$((SECONDS + 15))
    while [ $SECONDS -lt $end_time ]; do
        for url in "${endpoints[@]}"; do
        ...
        seq 1 30 | xargs -P 30 -I {} \
            curl -sk -o /dev/null "${BASE_URL}/health" 2>/dev/null
        wait
    done
    print_result "Phase 7 완료 — 시스템 정상 회복 확인"

    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}  종합 결과 — 각 모니터링 화면에서 확인할 것${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "  Scouter Client:"
    echo "  ┌──────────────────────────────────────────────────────┐"
    echo "  │ XLog        │ Phase1~5: 점 밀집, Phase6: 공백,      │"
    echo "  │             │ Phase7: 다시 밀집                      │"
    echo "  │ TPS         │ Phase4에서 최고점, Phase6에서 0        │"
    echo "  │ Heap Used   │ Phase4에서 급등, Phase6에서 GC 하락    │"
    echo "  │ Elapsed     │ Phase3에서 점진 증가, Phase4에서 최고  │"
    echo "  │ Active EQ   │ Phase3~4에서 막대 최대                 │"
    echo "  │ CPU         │ Phase4에서 스파이크                    │"
    echo "  └──────────────────────────────────────────────────────┘"
    echo ""
    echo "  Grafana (Middleware Overview):"
    echo "  ┌──────────────────────────────────────────────────────┐"
    echo "  │ CPU Usage        │ Phase4에서 스파이크               │"
    echo "  │ Memory Usage     │ 전체적으로 미세 증가              │"
    echo "  │ Network I/O      │ Phase5에서 TX 급등               │"
    echo "  │ Nginx Requests   │ Phase3~4에서 최고, Phase6에서 0  │"
    echo "  │ Nginx Connections│ Phase3에서 계단식 증가            │"
    echo "  │ JVM Heap (T1/T2)│ Phase4에서 급등, Phase6에서 하락  │"
    echo "  └──────────────────────────────────────────────────────┘"

    print_result "대시보드 종합 테스트 완료! (약 2분 30초 소요)"
}
...
case "$SCENARIO" in
    ...
    *)
        echo "사용법: $0 [all|health|mixed|slow|burst|failover|dashboard] [요청수] [동시성]"
        echo ""
        echo "  시나리오:"
        ...
        echo "    dashboard  모든 모니터링 그래프 활성화 (2분 30초)"
        echo "    all        시나리오 1~5 순차 실행"
        exit 1
        ;;
esac
